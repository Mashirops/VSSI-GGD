...
function [m,tau,ga,b,z,zu,nlZ,Q,T] = dli(X,y,s2,B,t,pot,tau, opts,G,K)
...
for i = 1:nout
  ...
  C1 = n*log(2*PI) - size(X,1)*log(2*PI*s2);
   nlZ(i) = ( phi0 + ldA - C1 - 2*size(z,1)*log(tau))/2;  
  if output, fprintf('NLZ(%s): %1.4e\n',innerType,nlZ(i)), end
  
  %% Added by Liu Ke on 2015/8/1, for MSE stor creterial
  %========================================================================
  MSE = abs((nlZ(i)-cost)/nlZ(i));
  if output, fprintf('MSE(%s): %1.4e\n',innerType,MSE), end
  cost = nlZ(i);
  %========================================================================
  ...
  if i<nout || nargout>3
    ...
    switch outerMethod
      ...
      case 'lanczos'
        ...
        % Added by Liu Ke on 2015/12/2
        args = {X(1:K:size(X,1),1:K:size(X,2)),1/s2,B(1:K:size(B,1),1:K:size(B,2)),pi(1:K:size(pi,1)),varOpts.MVM,output};
      case 'sample'
 ...
 %=========================================================================
  ...
  dpi = dev(pi_old,pi); db = dev(b_old,b); cvg = dpi+db<1e-6 || MSE <= 1e-2;
  if output
  ...
  if cvg || i==nout % converged or last loop
   ...
   %% tau updated; revised by Liu Ke on 2015/9/6
 %=========================================================================
%  alpha = 100; beta = 10000;
% tau = (alpha - 1)/ (1/beta + sum(sqrt(G*(z + (B*u).^2))))

%  tau = size(B,1)/K/ sum(sqrt(G*(z + (B*u).^2)))
  tau = size(B,1)/ sum(sqrt(G*(z + (B*u).^2)))
end
